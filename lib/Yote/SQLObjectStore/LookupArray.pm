...
sub ready {
    ...
    my ($min_idx, $max_idx) = $store->query_line( "SELECT min(idx), max(idx) FROM $table WHERE id=?", [$id] );
...
sub pop {
    ...
    return unless $self->{max_idx} > 0;

    $self->dirty;
    return $self->get( $self->{max_idx}-- );
}
