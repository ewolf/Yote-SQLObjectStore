...
sub test_stow_and_fetch_and_delete {
    ...
    is ($store->next_id, 2, 'second id' );
    is ($store->fetch(1), undef, 'nothing there for unused id' );
    $store->stow( "SOMETHING TO STOW", 2 );
    is( $store->fetch(2), "SOMETHING TO STOW", "stow given id" );

    if (defined $store->stow( "BADSTOW", 2.1 ) ) {
        fail( "was able to stow a non-integer" );
    }
    like ( $@, qr/must be a positive integer/, 'fail message for id 2.1' );
    
    if (defined $store->stow( "NOSTOW", 0 )) {
        fail( "was able to stow a non-positive integer" );
    } 
    like ( $@, qr/must be a positive/, 'fail message for id 0' );

    $@ = '';
    if (defined $store->stow( "NOSTOW", -2 )) {
        fail( "was able to stow a non-positive integer" );
    }
    like ( $@, qr/must be a positive/, 'fail message for id 0' );
    
    # 4096 - 5 is 4091 and 8192 - 5 is 8187, 16384 - 5 is 16379
    is ($store->stow( "x" x 8187, 2 ), 2, 'stow for two is two' );
    is ($store->stow( "y" x 4093 ), 3, 'nexly created thingo' );
...
#    is( $store->record_count, 2, "two records in store" );
    is( $store->get_record_count, 3, "3 entries in index" );
...
#    is( $store->record_count, 2, "still two entries" );
 ...
 #   is( $store->record_count, 3, "now 3 records" );
    is( $store->get_record_count, 4, "4th entry in index" );

    $store->stow( "tiny", 2 );
#    is( $store->record_count, 3, "3 records" );
    is( $store->get_record_count, 4, "still 4th entry in index" );

    is ($store->stow( "q" x 12 ), 5, 'next index' );
#    is( $store->record_count, 4, "now 4 records" );
    is( $store->get_record_count, 5, "5th entry in index" );

    my $del = $store->delete_record( 54 );
    ok ( defined($del) && $del == 0, 'nothign to delete' );
#    is( $store->record_count, 4, "now 4 records" );
    is( $store->get_record_count, 5, "5th entry in index" );

    $store = $rs_factory->new_rs;
    $store->stow( "FOO" );
    is( $store->get_record_count, 1, "ONE THING" );
    is ($store->fetch(1), "FOO", "thing started" );
    $store->stow( "BAR", 1 );
    is( $store->get_record_count, 1, "still ONE THING" );
    is ($store->fetch(1), "BAR", "thing changed" );
    
    my $id = $store->stow( "BAAAAAAAAGS" );
    $store->stow( "H", $id );
    is( $store->fetch( $id ), "H", "didn't pop out" );
    is( $store->get_record_count, 2, "2 entries" );
...
sub test_locks {
    ...
    if( defined $store->lock( "SOMETHING" ) ) {
...
sub test_recordstore {
    ...
    my $store = $rs_factory->new_rs;
    is( $store->get_record_count, 0, 'no entries in new store' );
    ...
    is( $store->get_record_count, 3, "store 3 entries" );
    ...
    is( $store->get_record_count, 2, "two entry count in store" );
#    is( $store->record_count, 2, "two record count in store" );
    ...
    is( $store->get_record_count, 3, "3 entry count in store" );
#    is( $store->record_count, 3, "3 record count in store" );

    $store->stow( "x" x 8188, $id );  # 12 is max 4092, 13 is max 8187

    is( $store->get_record_count, 3, "still 3 entry count in store" );
#    is( $store->record_count, 3, "still 3 record count in store" );
    ...
    is( $store->get_record_count, 3, "yet still 3 entry count in store" );
#    is( $store->record_count, 3, "yet still 3 record count in store" );

    my $xid = $store->stow( "x" x 90 );

    is( $store->get_record_count, 4, "now 4 entry count in store" );
#    is( $store->record_count, 4, "now 4 record count in store" );

    $store->delete_record( $id );

    is( $store->get_record_count, 4, "still 4 entry count in store after delete" );
#    is( $store->record_count, 3, "now 3 record count in store after delete" );

    $store = $rs_factory->new_rs;
    
    is( $store->get_record_count, 0, "empty then no entries" );
#    is( $store->record_count, 0, "empty then no records" );

    $store->next_id;
    $store->next_id;
    $store->next_id;
    is($store->stow( "BOOGAH"), 4, 'a store of 4');
    is( $store->next_id, '5', "next id is 5" );
    is( $store->get_record_count, 5, "5 entries after skipping ids plus asking to generate the next one" );
#    is( $store->record_count, 1, "one record at id 4" );
    ...
    is($store->stow( "TEN"), 10, "10 id" );
    is( $store->get_record_count, 10, "entry count explicitly set" );
#    is( $store->record_count, 2, "now 2 record count in store after delete after explicit set" );
    ...
    is( $store->get_record_count, 11, "entry count explicitly set" );
    is( $store->next_id, 12, 'next again' );
    is( $store->get_record_count, 12, "entry count explicitly set" );


    $store->stow( "x" x 90 );
    is( $store->get_record_count, 13, "now 13 entry count in store after stow" );
#    is( $store->record_count, 3, "now 3 record count in store after stow" );

    $xid = $store->stow( "x" x 90 );

    is( $store->get_record_count, 14, "now 14 entry count in store after stow" );
...
sub test_failed_async {
 ...
 #       $provider->commit_transaction(  );
        ...
        $forker->spush( '103' );
        $forker->expect( '104' );
#        $provider->use_transaction(  ); # cant use here due to mysql
        $forker->put( '105' );
        $forker->expect( '106' );
...
#        $provider->commit_transaction(  );
        ...
        $forker->spush( '100' );
        $forker->expect( '101' );
#        $provider->use_transaction(  ); #can't use here due to mysql
